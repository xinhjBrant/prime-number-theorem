...
import unit_fractions.misc
import unit_fractions.integral_rpow
/-
invalid import: for_mathlib.misc
could not resolve import: for_mathlib.misc
-/
...
/-! semi_normed_group as been renamed as seminormed_add_comm_group-/
lemma abs_summatory_le_sum {M : Type*} [seminormed_add_comm_group M] (a : ℕ → M) {k : ℕ} {x : ℝ} :
...
/-! semi_normed_group as been renamed as seminormed_add_comm_group-/
lemma abs_summatory_bound {M : Type*} [seminormed_add_comm_group M] (a : ℕ → M) (k z : ℕ)
  {x : ℝ} (hx : x ≤ z) :
...
/-! In the latest version of mathlib, the theorem "is_O_one_of_tendsto" has been replaced by a more general form "is_O_const_of_tendsto", therefore we give a new proof-/
...
begin
  apply is_O_const_of_tendsto hf.has_sum.tendsto_sum_nat _, simp
end
...
begin
  ...
  -- exact (is_O_const_of_tendsto _ ((real.summable_nat_rpow.2 this).has_sum.comp z)),
  apply (is_O_const_of_tendsto ((real.summable_nat_rpow.2 this).has_sum.comp z)),
  simp
end
...
begin
  ...
  -- exact (card_le_of_subset this).trans (by simp),
    exact (finset.card_le_of_subset this).trans (by simp),
end
...
/-! "lt_floor_add_one" is modified, the goal changes from a ≤ ↑⌊a⌋₊ + 1 to a ≤ ↑(⌊a⌋₊ + 1)-/
...
begin
  ...
  { rw [mul_comm, ←div_eq_mul_inv, le_div_iff' hx, ←div_eq_mul_inv, div_le_iff, ←pow_succ,
    ...
    { /- replace "exact (nat.lt_floor_add_one _).le" with the proof term of nat.lt_floor_add_one -/
      apply le_of_lt,
      by simpa using nat.lt_succ_floor (logb ↑p x)},
    { exact pow_pos hp₀ _, } },
